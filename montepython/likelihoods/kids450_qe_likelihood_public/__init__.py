...
#######################################################################
#
# Developed by F. Köhlinger based on and by largely adapting
# Benjamin Audren's Monte Python likelihood euclid_lensing
#
# To be used with data from F. Köhlinger et al. 2017 (MNRAS, 471, 4412)
...
class kids450_qe_likelihood_public(Likelihood):
    ...
    def __init__(self, path, data, command_line):
        ...
        for zbin in xrange(self.nzbins):
            ...
            self.redshifts = z_hist #np.linspace(z_sample.min(), z_sample.max(), self.nzmax, 'float64')
        ...
        if self.mode == 'halofit':
            self.need_cosmo_arguments(data, {'z_max_pk': self.z_max, 'output': 'mPk', 'non linear': self.mode, 'P_k_max_h/Mpc': 2*self.k_max})
        else:
            self.need_cosmo_arguments(data, {'z_max_pk': self.z_max, 'output': 'mPk', 'P_k_max_h/Mpc': 2*self.k_max})
