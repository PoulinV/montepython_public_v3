########################################################
# euclid_lensing_M010_IH likelihood
########################################################
...
class euclid_lensing_M010_IH(Likelihood):
	...
	#	print("%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s" % (self.z[nr], g[nr, 0], g[nr, 1], g[nr, 2], g[nr, 3], g[nr, 4], g[nr, 5], g[nr, 6], g[nr, 7], g[nr, 8], g[nr, 9]))
        ...
        #                "you should increase euclid_lensing_M010_IH.k_max up to at least %g" % (self.l[index_l]/self.r[index_z]))
...
#MArchi     for index_l in range(self.nlmax):
	    ...
	    if self.use_lmax_lincut:
			...
			try:
				...
				warnings.warn("euclid_lensing_M010_IH: Could not evaluate chi2 including theoretical error with the current parameters. The corresponding chi2 is now set to nan!")
				break
				break
				break
				chi2 = np.nan
